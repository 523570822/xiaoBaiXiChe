...
class AppsettingModel extends ModelService
{
    /**
     * 查询一条数据
     * @param $where
     ...
     */
    public function findAppsetting($where, $field= ''){
        if(empty($where)){
            return false;
        }else{
     ...
     */
    public function selectAppsetting($where = array(),$order = '',$field='',$p = '',$page_size = '',$parameter = array()){
        if($where['status'] == ''|| empty($where['status'])){
            $where['status'] = array('neq',9);
        }
        if($page_size == ''){
     ...
     */
    public function saveAppsetting($where,$data){
     ...
     * Date: 2018/6/13 8:44
     */
    public function deleteAppsetting($where, $del='del') {
        if ($del == 'remove') {
